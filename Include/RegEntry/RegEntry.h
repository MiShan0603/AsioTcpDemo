...
public:
	...
	void		SetOwner(CRegistry* Owner) { __cregOwner = Owner; }
	...
	bool		IsString()		{ return (iType == REG_SZ); }
	bool		IsDWORD()		{ return (iType == REG_DWORD); }
	bool		IsBinary()		{ return (iType == REG_BINARY); }
	bool		IsMultiString() { return (iType == REG_MULTI_SZ); }
	
	bool		IsStored()		{ return __bStored; }
	bool		Exists()		{ return __bStored; }

	void		MultiClear()	{ SetMulti(_T("\0"), 2); }
	void		MultiAdd(LPCTSTR lpszVal) { MultiSetAt(MultiCount(), lpszVal); }
